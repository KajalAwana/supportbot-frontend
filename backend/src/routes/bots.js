...
router.post('/', authenticate, async (req, res) => {
  ...
  try {
    ...
    const { count } = await supabase
      .from('bots').select('*', { count: 'exact', head: true }).eq('user_id', req.user.id);
...
router.put('/:id', authenticate, async (req, res) => {
  ...
  try {
    const { data: existing } = await supabase
      .from('bots').select('id').eq('id', req.params.id).eq('user_id', req.user.id).single();
...
// Public: get bot by embed key (for widget)
